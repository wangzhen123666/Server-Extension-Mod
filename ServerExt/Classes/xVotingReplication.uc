...
{
	if( S!="" )
		GetPlayer().ClientMessage("地图投票: "$S);
}
reliable client simulated function ClientNotifyVote( PlayerReplicationInfo PRI, int GameIndex, int MapIndex )
{
	if( bAllReceived )
		MapVoteMsg((PRI!=None ? PRI.PlayerName : "Someone")$" 投票 "$Maps[MapIndex].MapTitle$" ("$GameModes[GameIndex].GameShortName$").");
	else MapVoteMsg((PRI!=None ? PRI.PlayerName : "Someone")$" 投票了一张地图.");
}
...
reliable client simulated function ClientNotifyVoteWin( int GameIndex, int MapIndex, bool bAdminForce )
{
	...
	{
		if( bAllReceived )
			MapVoteMsg("管理员强制将地图切换为 "$Maps[MapIndex].MapTitle$" ("$GameModes[GameIndex].GameShortName$").");
		else MapVoteMsg("管理员强制切换地图.");
	}
	else if( bAllReceived )
		MapVoteMsg(Maps[MapIndex].MapTitle$" ("$GameModes[GameIndex].GameShortName$") 赢得投票, 切换地图中...");
	else MapVoteMsg("一张地图赢得投票, 切换地图中...");
}
reliable client simulated function ClientOpenMapvote( optional bool bShowRank )
{
	...
	if( bShowRank )
	{
		...
		if( KFGFxHudWrapper(GetPlayer().myHUD)!=None )
			KFGFxHudWrapper(GetPlayer().myHUD).HudMovie.DisplayPriorityMessage("地图投票时间","选择地图投票!",2);
