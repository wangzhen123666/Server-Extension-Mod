...
function DrawMenu()
{
	...
	Y+=YL;
	Canvas.SetPos(XPos+26,Y);
	Canvas.DrawText("时间: "$FormatTimeSM(KFGRI.ElapsedTime)$" | 玩家数: "$NumPlayer$" | 存活: "$NumAlivePlayer$" | 观看者: "$NumSpec,,FontScalar,FontScalar);
	...
	Y = YPos+4;
	DrawCenteredText("尸潮波数",XPos+XScale*0.5,Y,FontScalar);
	Y += YL;
...
function DrawPlayerEntry( Canvas C, int Index, float YOffset, float Height, float Width, bool bFocus )
{
	...
	if( !bShowSpectatorsOnly )
	{
		...
		{
			C.DrawColor = SBTextColor;
			S = "重生: "$FormatTimeSM(PRI.RespawnCounter);
		}
		while( true ) // Make sure too long name doesn't overleap.
		{
			C.TextSize(S,XL,YL,YPos,YPos);
...
function SelectedRCItem( int Index )
{
	...
	case 2: // Mute voice.
		...
		{
			PC.ClientMessage("你静音了 "$RightClickPlayer.TaggedPlayerName);
			PC.ClientMutePlayer(RightClickPlayer.UniqueId);
			RightClickPlayer.bIsMuted = true;
		}
		else
		{
			PC.ClientMessage("你关闭静音 "$RightClickPlayer.TaggedPlayerName);
