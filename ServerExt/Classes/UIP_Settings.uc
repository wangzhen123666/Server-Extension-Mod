...
function InitMenu()
{
	...
	AddCheckBox("隐藏名称信标:","隐藏玩家名字信标",'NB',class'ExtPlayerController'.Default.bHideNameBeacons);
	...
	AddCheckBox("隐藏浮动伤害信息:","隐藏浮动伤害信息",'PP',class'ExtPlayerController'.Default.bHideNumberMsg);
	...
	KeyBindButton = AddButton("","视角切换按键:","这个按钮可以切换视角(点击改变它)",'KB',KeyBindLabel);
	AddCheckBox("不变成丧尸:","禁用丧尸玩家模式(需要游戏模式支持)",'ZP',class'ExtPlayerController'.Default.bNoMonsterPlayer);
	AddCheckBox("没有屏幕抖动:","没有屏幕抖动 (来自爆炸)",'NS',class'ExtPlayerController'.Default.bNoScreenShake);
	InitBehindviewKey();
}
